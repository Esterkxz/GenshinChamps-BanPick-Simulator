locales["en-global"] = {
    "comment": "\
English for Global\n\
default language\n\
This language works by Esterisk (에스터1z / Ester1z)\n\
    ",
    "name": "🌐English",
    "text": {

        "sideRed": "RED",
        "sideBlue": "BLUE",

        "pick": "PICK",
        "ban": "BAN",

        "pickBan": "BAN",
        "pickEntry": "Party ENTRY",
        "pickProffer": "PROFFER Entry",
        "pickBanWeapon": "BAN Weapons",
        "pickUseBanCard": "target for use BAN CARD",

        "pickedBan": "BAN",
        "pickedEntry": "PICK",
        "pickedEntryShort": "P",
        "banCardTitle": "BAN",
        "banCardTitleShort": "B",

        "pickCursorBan": "Pick",
        "pickCursorEntry": "Jail",
        "pickCursorBanWeapon": "Shut out",

        "pickUnallowed": "Unallowed",

        "entryCostDesc": "Left click or Wheel up: Increase one additional cost\nRight click or Wheel down: Decrease one additional cost",
        "entryBanCardDesc": "Hands with the Ban card",

        "pisPlayerProfileSelectDesc": "Click: Select profile character of player\Right-click: Remove profile character view",
        "pisPlayerInfoClearDesc": "Click: Clear weapon and refine entered for this player\nRight-click: Clear whole player infos for this player",
        "pisName": "Name",
        "pisLumine": "Lumine",
        "pisAether": "Aether",
        "pisCopyAccountCode": "Copy account data",
        "pisCopyAccountCodeDesc": "Click: Copy to clipboard for player account info data",
        "pisDataAccountCode": "Account info data\n(paste or auto generate)",
        "pisDataAccountCodeDesc": "Paste: Load account info data(blind when before versus)\nTo be auto generated account info code by changes player info values",
        "pisAddTimeConstell": "Con",
        "pisAddTimeConstellDesc": "Add times by 5-star character picked and each constells of #SIDE side",
        "pisAddTimeHasWapon": "Wep",
        "pisAddTimeHasWaponDesc": "Add times by 5-star weapon euipped of #SIDE side",
        "pisAddTimeWeponRefine": "Ref",
        "pisAddTimeWeponRefineDesc": "Add times by 5-star weapon's each additional refinements of #SIDE side",
        "pisAddTimeCommonTails": "Default(is this empty): #SECsec\nRight-click: Initialize(be empty=to default)\nTo be applied each side when changed values this item on hold Shift key",
        "pisAddTimeDisadvRatio": "Rat",
        "pisAddTimeDisadvRatioDesc": "Differ apply ratio of account point disadvantage for #SIDE side if higher A.P. proffer player\nRight-click: Initialize(to default, default: 0.5)\nTo be applied each side when changed values this item on hold Shift key",
        "pisAddTimeAdjust": "Adj",
        "pisAddTimeAdjustDesc": "Adjust master add times of #SIDE side (Default: 0)\nRight-click: Initialize(to default)\nWheel Up/Down: Adjust master time add value",
        "pisAddsTotal": "Total",

        "pssCharConstellDesc": "Enter character entry's constellation\nRight-click: To be empty",
        "pssCharName": "Entry #NO",
        "pssCharNameDesc": "Character's name of entry (to be auto entered when selected entry)",
        "pssWeaponName": "Weapon #NO",
        "pssWeaponNameDesc": "Enter weapon's name(shorter) equipped the character of this entry",
        "pssWeaponRefineDesc": "Enter this weapon's refinement\nRight-click: To be empty",

        "sum": "SUM",
        "unitMin": "m",
        "unitSec": "s",

        "valueSignatureWeapon": "S.W.",
        "valueSignatureWeaponAliases": "Signature|sign.",

        "vsVersus": "VERSUS",
        "vsRemains": "Time Ramains",
        "vsElapsed": "Time Elapsed",
        "vsOptionTKO": "Select TKO",
        "vsHalf1st": "1st Half",
        "vsHalf2nd": "2nd Half",
        "vsBtnTkoByTimeover": "Timeover",
        "vsBtnTkoByPowerloss": "Powerloss",
        "vsBtnTkoBySurrender": "Surrender/Etc.",
        "vsTkoHalf": "#HALF",
        "vsTkoByTimeover": "TIMEOVER",
        "vsTkoByPowerloss": "POWERLOSS",
        "vsTkoBySurrender": "SURRENDER/ETC.",
        "vsAdds": "Time Adds",
        "vsTotalElapsed": "Total Elapsed",
        "vsChamber": "Chamber #NO",
        "vsAggregated": "AGGREGATED",
  
        "btnStart": "START",
        "btnPass": "PASS",
        "btnVersus": "VERSUS",
        "btnUnstopable": "UNSTOPABLE",
        "btnMainDesc": "Progress next thing",
        "btnUndo": "UNDO",
        "btnUndoDesc": "Return to previous step and release last picked",
        "btnRemoconDesc": "Use remote controller (popup window)",
        "btnRandomDesc": "Click: Pick one randomly\nRight click: Auto pick remains randomly.\n stopable with click [!] button",
        "btnPlayerInfoToggleDesc": "Show/hide player info operator control panel",
        "btnSettings": "settings",
        "btnSettingsDesc": "Switch light/dark UI mode in current version",
        "btnReset": "reset",
        "btnResetDesc": "Initialize whole picked and entry user names",
        "btnLang": "Languages",
        "btnLangDesc": "Change UI language\nCannot change language after pick up progress be started",

        "searchPlaceholder": "search (weapon only)",
        "searchDesc": "To be updated search results by each inputs\nKeyboard ↑/↓/PgUp/PgDn/Home/End: Move cursor in list of search result\nEnter: Pick up current item with cursor\nMouse left click item from list: Pick up item\nEsc: Initialize search inputs - see whole list",

        "playerProfileSelection": "Select character for player #SIDE's profile illust to show",

        "readyForStart": "Press START button",
        "readyForVersus": "Press VERSUS button to start for game",

        "titleReady": "Ready",
        "stepTitle": "<span class=\"#CLASS\">#SIDE</span> Pick #AMOUNT for #TYPE",
        "amountPick": "#AMOUNT weapon(s)",
        "amountPickCharacter": "#AMOUNT character(s)",
        "amountFillCharacter": "character(s) place to fill",
        "titlePicked": "Pick finished",
        "titleVersus": "VERSUS",
        "titleLeagueMatch": "#NAME LEAGUE MATCH",

        "setConstellationDesc": "Set constellation",

        "soundMuteDesc": "Mute",

        "timerDisplayDesc": "Click: Start / stop and reset the timer\nRight-click:\n - Before the timer starts: Reset the increase/decrease application of the current timer\n - After the timer starts: Pause/resume the timer\nWheel Up/Down: Adjust the default timer time (only available when stopped/paused)\nWheel Click: Return the default timer time to the default value (30 seconds) even when paused/operating",
        "timerAdderMinDesc": "Click: Add #NO minutes (limited to current counter)\nRight-click: Reduce #NO minutes (limited to current counter)\nWheel Up/Down: Increase/decrease by 1 second (limited to current counter)\nWheel click: Reset current counter increase/decrease",
        "timerAdderSecDesc": "Click: Add #NO seconds (limited to current counter)\nRight-click: Reduce #NO seconds (limited to current counter)\nWheel Up/Down: Increase/decrease by 1 second (limited to current counter)\nWheel click: Reset current counter increase/decrease",
        "timerOptionsSetSoundDesc": "Set timer sound",
        "timerOptionSoundOnDesc": "Timer start/stop/reset sound On",
        "timerOptionCounterSoundOnDesc": "Timer counter sound On\n - Every 2 seconds when 15 seconds or more\n - Every 1 second when less than 15 seconds\n - Sound changed when less than 10 seconds\n - Add .5 seconds when less than 5 seconds",
        "timerOptionControlSoundVolumeDesc": "Timer start/stop/reset sound volume",
        "timerOptionCounterSoundVolumeDesc": "Timer counter sound volume",
        "timerOptionSoundVolumeDefaultDesc": "Click: set default value (#VALUE)",
        "timerOptionsSetInterlockDesc": "Set up timer pickup interlock",
        "timerOptionInterlockSideOnDesc": "Side pickup interlock On\n - Connect timer color and activate top timer gauge on the screen\n - Automatically reset the timer when the pickup side changes",
        "timerOptionInterlockAmountOnDesc": "Amount pickup interlock On\n - Automatically apply default period*number of pickups by pickup sequence step",
        "timerOptionsSetAutomaticDesc": "Set automatic timer start and timeout pass",
        "timerOptionAutomaticStartBanOnDesc": "Ban pick timer automatic start On",
        "timerOptionAutomaticStartEntryOnDesc": "Entry pick timer automatic start On",
        "timerOptionAutomaticPassRanOutOnDesc": "Automatic pass when time runs out",
        "timerOptionsSetSetupPhaseDesc": "Set player party setting time timer",
        "timerOptionAutomaticStartSetupPhaseOnDesc": "Automatic start of setting time timer On",
        "timerOptionDefaultTimeoutSetupPhaseTextDesc": "Setting time timer default value",
        "timerOptionDefaultTimeoutSetupPhaseMinDesc": "Setting time timer default value (minutes - maximum 16 minutes 39 seconds == 999 seconds)",
        "timerOptionDefaultTimeoutSetupPhaseSecDesc": "Setting time timer default value (seconds - maximum 16 minutes 39 seconds == 999 seconds)",
        "timerSettingsIconDesc": "Click: Toggle close/open setting panel fixed mode\nRight-click: Show on mouse over (default state)",

        "accountInfoIndicatorDesc": "Set of account information indicators - 🟩 No information / ✅ Information available\nClick:\n If there is information - Delete information\n If there is no information - Reset the entered name/UID/account score\nRight-click: Restore previous account information",

        "reserved": ""
    },
    "class": {
        "ordinary": "Ordinary",
        "limited": "Limited",
        "event": "Event"
    },
    "weapon": {
        "sword": "Sword",
        "claymore": "Claymore",
        "polearm": "Polearm",
        "bow": "Bow",
        "catalyst": "Catalyst"
    },
    "weapon_aliases": {
        "sword": ["blade"],
        "claymore": [""],
        "polearm": ["spear"],
        "bow": [""],
        "catalyst": ["book"]
    },
    "element": {
        "anemo": "Anemo",
        "geo": "Geo",
        "pyro": "Pyro",
        "cryo": "Cryo",
        "hydro": "Hydro",
        "electro": "Electro",
        "dendro": "Dendro",
        "switchable": "Switchable"
    },
    "continent": {
        "mondstadt": "Mondstadt",
        "liyue": "Liyue",
        "inazuma": "Inazuma",
        "sumeru": "Sumeru",
        "fontaine": "Fontaine",
        "natlan": "Natlan",
        "snezhnaya": "Snezhnaya",
        "descendent": "Descendent",
        "castaway": "Castaway"
    },
    "characters": {
        "treveler": "Traveler",
        "amber": "Amber",
        "kaeya": "Kaeya",
        "lisa": "Lisa",
        "babara": "Barbara",
        "razor": "Razor",
        "xiangling": "Xiangling",
        "beidou": "Beidou",
        "xingqiu": "Xingqiu",
        "ningguang": "Ningguang",
        "fischl": "Fischl",
        "bennett": "Bennett",
        "noelle": "Noelle",
        "chongyun": "Chongyun",
        "sucrose": "Sucrose",
        "jean": "Jean",
        "diluc": "Diluc",
        "qiqi": "Qiqi",
        "mona": "Mona",
        "keqing": "Keqing",
        "venti": "Venti",
        "klee": "Klee",
        "diona": "Diona",
        "tartaglia": "Tartaglia",
        "xinyan": "Xinyan",
        "zhongli": "Zhongli",
        "albedo": "Albedo",
        "ganyu": "Ganyu",
        "xiao": "Xiao",
        "hutao": "Hu Tao",
        "rosaria": "Rosaria",
        "yanfei": "Yanfei",
        "eula": "Eula",
        "kazuha": "Kaedehara Kazuha",
        "ayaka": "Kamisato Ayaka",
        "sayu": "Sayu",
        "yoimiya": "Yoimiya",
        "aloy": "Aloy",
        "sara": "Kujou Sara",
        "shogun": "Raiden Shogun",
        "kokomi": "Sangonomiya Kokomi",
        "thoma": "Thoma",
        "gorou": "Gorou",
        "itto": "Arataki Itto",
        "yunjin": "Yun Jin",
        "shenhe": "Shenhe",
        "yae": "Yae Miko",
        "ayato": "Kamisato Ayato",
        "yelan": "Yelan",
        "shinobu": "Kuki Shinobu",
        "heizo": "Shikanoin Heizou",
        "collei": "Collei",
        "tighnari": "Tighnari",
        "dori": "Dori",
        "candace": "Candace",
        "cyno": "Cyno",
        "nilou": "Nilou",
        "nahida": "Nahida",
        "layla": "Layla",
        "faruzan": "Faruzan",
        "wanderer": "Wanderer",
        "yaoyao": "Yaoyao",
        "alhaitham": "Alhaitham",
        "dehya": "Dehya",
        "mika": "Mika",
        "kaveh": "Kaveh",
        "baizhu": "Baizhu",
        "kirara": "Kirara",
        "lynette": "Lynette",
        "lyney": "Lyney",
        "freminet": "Freminet",
        "wriothesley": "Wriothesley",
        "wriothesley": "Wriothesley",
        "neuvillette": "Neuvillette",
        "charlotte": "Charlotte",
        "furina": "Furina",
        "chevreuse": "Chevreuse",
        "navia": "Navia",
        "gaming": "Gaming",
        "liuyun": "Xianyun",
        "chiori": "Chiori",
        "reserved": ""
    },
    "characters_short": {
        "treveler": "Traveler",
        "amber": "Amber",
        "kaeya": "Kaeya",
        "lisa": "Lisa",
        "babara": "Barbara",
        "razor": "Razor",
        "xiangling": "Xiangling",
        "beidou": "Beidou",
        "xingqiu": "Xingqiu",
        "ningguang": "Ningguang",
        "fischl": "Fischl",
        "bennett": "Bennett",
        "noelle": "Noelle",
        "chongyun": "Chongyun",
        "sucrose": "Sucrose",
        "jean": "Jean",
        "diluc": "Diluc",
        "qiqi": "Qiqi",
        "mona": "Mona",
        "keqing": "Keqing",
        "venti": "Venti",
        "klee": "Klee",
        "diona": "Diona",
        "tartaglia": "Tartaglia",
        "xinyan": "Xinyan",
        "zhongli": "Zhongli",
        "albedo": "Albedo",
        "ganyu": "Ganyu",
        "xiao": "Xiao",
        "hutao": "Hu Tao",
        "rosaria": "Rosaria",
        "yanfei": "Yanfei",
        "eula": "Eula",
        "kazuha": "Kazuha",
        "ayaka": "Ayaka",
        "sayu": "Sayu",
        "yoimiya": "Yoimiya",
        "aloy": "Aloy",
        "sara": "Sara",
        "shogun": "Raiden",
        "kokomi": "Kokomi",
        "thoma": "Thoma",
        "gorou": "Gorou",
        "itto": "Itto",
        "yunjin": "Yun Jin",
        "shenhe": "Shenhe",
        "yae": "Yae Miko",
        "ayato": "Ayato",
        "yelan": "Yelan",
        "shinobu": "Shinobu",
        "heizo": "Heizou",
        "collei": "Collei",
        "tighnari": "Tighnari",
        "dori": "Dori",
        "candace": "Candace",
        "cyno": "Cyno",
        "nilou": "Nilou",
        "nahida": "Nahida",
        "layla": "Layla",
        "faruzan": "Faruzan",
        "wanderer": "Wanderer",
        "yaoyao": "Yaoyao",
        "alhaitham": "Alhaitham",
        "dehya": "Dehya",
        "mika": "Mika",
        "kaveh": "Kaveh",
        "baizhu": "Baizhu",
        "kirara": "Kirara",
        "lynette": "Lynette",
        "lyney": "Lyney",
        "freminet": "Freminet",
        "wriothesley": "Wriothesley",
        "neuvillette": "Neuvillette",
        "charlotte": "Charlotte",
        "furina": "Furina",
        "chevreuse": "Chevreuse",
        "navia": "Navia",
        "gaming": "Gaming",
        "liuyun": "Xianyun",
        "chiori": "Chiori",
        "reserved": ""
    },
    "weapons": {
        "sw_dvalin": { "name": "Skyward Blade", "aliases": [""] },
        "cm_dvalin": { "name": "Skyward Pride", "aliases": [""] },
        "pl_dvalin": { "name": "Skyward Spine", "aliases": [""] },
        "bw_dvalin": { "name": "Skyward Harp", "aliases": [""] },
        "ct_dvalin": { "name": "Skyward Atlas", "aliases": [""] },
        "sw_falcon": { "name": "Aquila Favonia", "aliases": [""] },
        "cm_wolfmound": { "name": "Wolf's Gravestone", "aliases": [""] },
        "bw_amos": { "name": "Amos' Bow", "aliases": [""] },
        "ct_fourwinds": { "name": "Lost Prayer to the Sacred Winds", "aliases": [""] },
        "sw_kunwu": { "name": "Summit Shaper", "aliases": [""] },
        "cm_kunwu": { "name": "The Unforged", "aliases": [""] },
        "pl_kunwu": { "name": "Vortex Vanquisher", "aliases": [""] },
        "bw_kunwu": { "name": "Kunwu's Wyrmbane", "aliases": [""] },
        "ct_kunwu": { "name": "Memory of Dust", "aliases": [""] },
        "sw_morax": { "name": "Primordial Jade Cutter", "aliases": [""] },
        "cm_morax": { "name": "Primordial Jade Greatsword", "aliases": [""] },
        "pl_morax": { "name": "Primordial Jade Winged-Spear", "aliases": [""] },
        "bw_morax": { "name": "Primordial Jade Vista", "aliases": [""] },
        "ct_morax": { "name": "Jadefall's Splendor", "aliases": ["Primordial", "Jade", "Regalia"] },
        "pl_homa": { "name": "Staff of Homa", "aliases": [""] },
        "sw_widsith": { "name": "Freedom-Sworn", "aliases": [""] },
        "cm_widsith": { "name": "Song of Broken Pines", "aliases": [""] },
        "pl_widsith": { "name": "?", "aliases": [""] },
        "bw_widsith": { "name": "Elegy for the End", "aliases": [""] },
        "ct_widsith": { "name": "?", "aliases": [""] },
        "sw_narukami": { "name": "Mistsplitter Reforged", "aliases": [""] },
        "pl_narukami": { "name": "Engulfing Lightning", "aliases": [""] },
        "bw_narukami": { "name": "Thundering Pulse", "aliases": [""] },
        "ct_narukami": { "name": "Kagura's Verity", "aliases": [""] },
        "bw_worldbane": { "name": "Polar Star", "aliases": [""] },
        "cm_itadorimaru": { "name": "Redhorn Stonethresher", "aliases": [""] },
        "pl_santika": { "name": "Calamity Queller", "aliases": [""] },
        "ct_kaleido": { "name": "Everlasting Moonglow", "aliases": [""] },
        "sw_amenoma": { "name": "Haran Geppaku Futsu", "aliases": [""] },
        "bw_kirin": { "name": "Aqua Simulacra", "aliases": [""] },
        "sw_ayus": { "name": "Light of Foliar Incision", "aliases": [""] },
        "bw_ayus": { "name": "Hunter's Path", "aliases": [""] },
        "ct_ayus": { "name": "A Thousand Floating Dreams", "aliases": [""] },
        "sw_deshret": { "name": "Key of Khaj-Nisut", "aliases": [""] },
        "cm_deshret": { "name": "Beacon of the Reed Sea", "aliases": [""] },
        "pl_deshret": { "name": "Staff of the Scarlet Sands", "aliases": [""] },
        "ct_alaya": { "name": "Tullaytullah’s Remembrance", "aliases": [""] },
        "bw_pledge": { "name": "The First Great Magic", "aliases": [""] },
        "ct_wheatley": { "name": "Cashflow Supervision", "aliases": [""] },
        "ct_iudex": { "name": "Rite of the Eternal Flow", "aliases": [""] },
        "sw_regalis": { "name": "Splendor of Tranquil Waters", "aliases": [""] },
        "cm_goldenverdict": { "name": "Verdict", "aliases": [""] },
        "ct_mountaingale": { "name": "Crane's Echoing Call", "aliases": [""] },
        "sw_needle": { "name": "Uraku Misugiri", "aliases": [""] },
        "reserved": { "name": "", "aliases": [""] }
    }
}